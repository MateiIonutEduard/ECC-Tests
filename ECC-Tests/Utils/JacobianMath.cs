...
namespace ECC_Tests.Utils
{
    public static class JacobianMath
    {
        public static JacobianPoint Add(EllipticCurve curve, JacobianPoint left, JacobianPoint right)
        {
            ...
            BigInteger A3 = (left.y * (((right.z * right.z) % p) % p) * right.z) % p;
            BigInteger A4 = (right.y * ((left.z * left.z) % p) * left.z) % p;
        ...
        {
            if (jacobianPoint.z == 0) return JacobianPoint.POINT_INFINITY;
            BigInteger p = curve.field;
